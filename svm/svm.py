...
class SVM(object):
    ...
    def _select_two_parameters(self):
        ...
        i1_list_1 = filter(lambda i: 0 < self.alpha[i] < self.C, index_list)
    ...
    def train(self, features, labels):
        ...
        for times in range(self.Max_Interation):
            ...
            eta = self._K_(self.X[i1], self.X[i1]) + self._K_(self.X[i2], self.X[i2]) \
                  - 2 * self._K_(self.X[i1], self.X[i2])  # 公式(7.107)
            ...
            b1_new = -E1 - self.Y[i1] * self._K_(self.X[i1], self.X[i1]) * (alph1_new - self.alpha[i1]) \
                     - self.Y[i2] * self._K_(self.X[i2], self.X[i1]) * (alph2_new - self.alpha[i2]) + self.b

            b2_new = -E2 - self.Y[i1] * self._K_(self.X[i1], self.X[i2]) * (alph1_new - self.alpha[i1]) \
                     - self.Y[i2] * self._K_(self.X[i2], self.X[i2]) * (alph2_new - self.alpha[i2]) + self.b
